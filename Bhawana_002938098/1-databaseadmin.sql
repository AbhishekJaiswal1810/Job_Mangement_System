...
begin
    ...
    if (userexist = 0) then
        ...
        execute immediate 'grant create trigger, alter trigger, drop trigger to DMDDjobportalsysadmin';
        execute immediate 'grant drop user to DMDDjobportalsysadmin with admin option';
        execute immediate 'grant create trigger, alter any trigger, drop any trigger to DMDDjobportalsysadmin with admin option';
        execute immediate 'grant create any index, alter any index, drop any index to DMDDjobportalsysadmin with admin option';
    end if;
end;
/
